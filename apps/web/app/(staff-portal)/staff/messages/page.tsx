...
export default function StaffMessagesPage() {
  ...
  const { data: messages, isLoading } = useQuery({
    ...
    queryFn: () => api.get<Message[]>('/api/messages?limit=50'),
    refetchInterval: 15000,
  });

  const { data: thread, isLoading: loadingThread } = useQuery({
    queryKey: ['staff-thread', selectedPatientId],
  ...
  const sendMutation = useMutation({
    mutationFn: (body: string) =>
      api.post('/api/messages/send', { patientId: selectedPatientId, body, channel: 'WHATSAPP' }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['staff-thread', selectedPatientId] });
      setReplyText('');
      toast.success('Sent');
    },
  ...
  const threads = messages?.reduce<Record<string, { patient: Message['patient']; lastMsg: Message; unread: number }>>(
    (acc, msg) => {
  ...
  return (
    <div className="space-y-4 h-[calc(100vh-8rem)]">
      ...
      <div className="flex gap-4 h-[calc(100%-4rem)]">
        ...
        <div className={cn(
          'card flex flex-col',
          selectedPatientId ? 'hidden md:flex w-72 flex-shrink-0' : 'flex flex-1 md:w-72 md:flex-shrink-0 md:flex-grow-0'
        )}>
          <div className="p-3 border-b border-border">
            <p className="text-xs font-bold text-muted">All Conversations</p>
          </div>
        ...
        {selectedPatientId ? (
          <div className="card flex flex-col flex-1 min-w-0">
            <div className="p-4 border-b border-border flex items-center gap-3">
              <button onClick={() => setSelectedPatientId(null)} className="md:hidden text-muted mr-1" aria-label="Back">←</button>
              ...
              {loadingThread
                ? [...Array(4)].map((_, i) => (
                    <div key={i} className={cn('flex', i % 2 === 0 ? 'justify-start' : 'justify-end')}>
                      <div className="skeleton h-12 w-48 rounded-[14px]" />
                    </div>
                  ))
                : thread?.messages.map((msg) => (
                    <div key={msg.id} className={cn('flex', msg.direction === 'OUTBOUND' ? 'justify-end' : 'justify-start')}>
                      <div className={cn(
                        ...
                        msg.direction === 'OUTBOUND' ? 'bg-brand text-white rounded-br-sm' : 'bg-subtle text-primary rounded-bl-sm'
                      )}>
                        <p className="leading-relaxed">{msg.body}</p>
                        <div className={cn('flex items-center gap-1.5 mt-1',
                          msg.direction === 'OUTBOUND' ? 'justify-end' : 'justify-start'
                        )}>
                          <p className={cn('text-[10px]', msg.direction === 'OUTBOUND' ? 'text-white/70' : 'text-faint')}>
            ...
            {thread?.aiSuggestion && (
              <div className="mx-4 mb-2 bg-brand-light rounded-btn px-3 py-2 flex items-center justify-between gap-3">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <p className="text-xs font-semibold text-brand truncate">AI: {thread.aiSuggestion}</p>
                </div>
                <button onClick={() => setReplyText(thread.aiSuggestion ?? '')} className="text-xs font-bold text-brand flex-shrink-0">
                  Use →
                </button>
              </div>
            )}
